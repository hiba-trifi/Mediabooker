...
if (isset($_POST["connecter"])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $errors["mot_pass"] = "Veuillez saisir votre mot de passe";
    } else {
        $stmt_email = $pdo->prepare("SELECT * FROM membre WHERE email_mb = ?");
        $stmt_email->execute([$email]);
        $row_l = $stmt_email->fetch(PDO::FETCH_ASSOC);

        $stmt_mot_pass = $pdo->prepare("SELECT * FROM membre WHERE email_mb = ?");
        $stmt_mot_pass->execute([$email]);
        $row_p = $stmt_mot_pass->fetch(PDO::FETCH_ASSOC);

        if (!($row_l) || password_verify($mot_pass, $row_l["mdp_mb"]) === false) {

            $errors["mot_pass_inc"] = "  Mot de passe saisi est un incorrect";
        } else {
            $_SESSION["Email"] = $email;
            $_SESSION["Id"] = $row_l["id_mb"];
            $_SESSION["nom"] = $row_l["nom_mb"];
            $_SESSION["surnom"] = $row_l["surnom_mb"];
            $_SESSION["MotPasse"] = $mot_pass;
            $_SESSION["tel"] = $row_l["tel_mb"];
            ...
            <div class="collapse navbar-collapse d-flex justify-content-end" id="navbarNav">
                <ul class="navbar-nav  ">
                    <li class="nav-item mx-3 fw-bold fs-5  ">
                        <a class="nav-link " href="./landing_page_user.html">Réseaux sociaux</a>
                    </li>
                    <li class="nav-item mx-3 fw-bold fs-5  ">
                        <a class="nav-link " href="./landing_page_user.html">Galerie</a>
                    </li>
                    <li class="nav-item mx-3  fw-bold fs-5  ">
                        <a class="nav-link  " aria-current="page" href="./landing_page_user.html">À propos</a>
                    </li>
                ...
                <form class=" mx-5" action="" method="POST">
                    ...
                    <div class=" d-flex flex-column justify-content-center align-items-center mt-5 ">
                        ...
                        <div class="d-flex">
                            ...
                            <a class="form_toogle mx-2" href="inscription.php">Inscrivez-vous ici</a>
                        </div>
                    </div>
                </form>
            </div>
